...
class MLPerfTrainTemplate(BertTemplate):
  ...
  def ProgramSchedule(self):
    ...
    p.ml_perf.decoder_metric_success_threshold = 0.8
...
@model_registry.RegisterSingleTaskModel
class MLPerfBertDense13B32x32SingleStepReshaped(MLPerfBertDense13B32x32SingleStep):
  BATCH_SIZE = 2048
  DEVICE_MESH_SHAPE = [256, 8]
  DEVICE_MESH = np.reshape(
      np.arange(0, np.product(DEVICE_MESH_SHAPE)), [32, 64]).transpose()
...
@model_registry.RegisterSingleTaskModel
class MLPerfBertDense13B16x32(MLPerfBertDense13B32x32):
  DEVICE_MESH_SHAPE = [64, 16]
  device_mesh =  np.reshape(
        np.arange(0, np.product(DEVICE_MESH_SHAPE)), [16, 64])
  DEVICE_MESH = gshard_utils.ZigzagOrderOnDeviceMesh(device_mesh, zigzag_mesh_dim=0).transpose()