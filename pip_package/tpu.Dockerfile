...
RUN for python in python3.8; do \
      ...
      bazel build --config=opt --distinct_host_configuration=true --define=framework_shared_object=true --define=with_tpu_support=true --copt=-DLIBTPU_ON_GCE -D_LIBGCXX_USE_CXX14_ABI=0 -std=c++14 //tensorflow/tools/pip_package:build_pip_package && \
